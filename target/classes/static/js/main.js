    ...
    if (checkoutForm) {
        ...
        checkoutForm.addEventListener('submit', async (e) => {
            ...
            if (method === 'COD') {
                ...
                const rzpRes = await fetch('/api/checkout/create-razorpay-order', { method:'POST', headers:{'Content-Type':'application/json','Authorization':`Bearer ${token}`}, body:JSON.stringify(payload) });
                if (!rzpRes.ok) { showPopup('Failed to create Razorpay order', 'Error', 'error'); btn.disabled = false; btn.innerHTML = 'Place Order'; return; }
                const d = await rzpRes.json();
                new Razorpay({ key:'rzp_test_dummykey12345', amount:d.amount*100, currency:'INR', name:'CafeBrew', order_id:d.orderId,
                    handler: async (resp) => {
                        payload.razorpayOrderId = resp.razorpay_order_id; payload.razorpayPaymentId = resp.razorpay_payment_id;
                        const fr = await fetch('/api/checkout/submit', { method:'POST', headers:{'Content-Type':'application/json','Authorization':`Bearer ${token}`}, body:JSON.stringify(payload) });
                        if (fr.ok) { localStorage.removeItem('cafebrew_cart'); window.location.href = '/my-orders'; }
                    }, theme:{color:'#cbb26a'} }).open();
